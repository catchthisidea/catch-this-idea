...
function buildHtml(name, email, lang, title, message) {
  ...
  <div style="background:#fff0e0;border-left:4px solid #e86000;padding:1rem 1.2rem;border-radius:4px;margin-bottom:1.5rem">
    <strong style="color:#e86000">Nova sugestão recebida</strong>
  </div>
  <table style="width:100%;border-collapse:collapse;font-size:14px">
    ...
    <tr><td style="padding:.5rem 0;color:#7a6040;vertical-align:top">Título</td><td style="padding:.5rem 0"><strong>${safeTitle}</strong></td></tr>
...
export default async (req) => {
  ...
  if (!safeTitle)   return json({ error: 'O título é obrigatório.' }, 400, origin);
  if (!safeMessage) return json({ error: 'A mensagem é obrigatória.' }, 400, origin);

  const code    = langLabel[lang] || 'PT';
  const subject = `[Suggestions][${code}] - ${safeTitle}`;
